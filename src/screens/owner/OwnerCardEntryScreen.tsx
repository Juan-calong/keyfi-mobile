// screens/owner/OwnerCardEntryScreen.tsx (SOP + BB/CIELO, sem MP)
...
export function OwnerCardEntryScreen({ route, navigation }: any) {
  const orderId: string | undefined = route?.params?.orderId;
  const cardToken: string | undefined = route?.params?.cardToken;
  ...
  const orderQ = useQuery({
    queryKey: ["owner-card-entry-order", orderId],
    enabled: !!orderId,
    ...
    return (
      <Screen>
        <Container>
          ...
          <Text style={{ marginTop: 8 }}>Volte e tokenize novamente o cartão.</Text>

          <Pressable
            onPress={() => navigation.replace(OWNER_SCREENS.CardTokenize, { orderId})}
      ...
      <Container style={{ flex: 1, paddingTop: 6 }}>
        ...
        <ScrollView contentContainerStyle={m.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <Card style={m.cardGroup}>
            ...
            <Text style={m.small}>Token recebido com sucesso. (CVV ainda é necessário para autorizar)</Text>
            ...
            <View style={{ gap: 10 }}>
              {installmentOptions.map((n) => (
                <Pressable
                  ...
                  style={({ pressed }) => [m.option, pressed && { opacity: 0.9 }, n === installments && { borderColor: "#000" }]}
                >
                  <Text style={m.optionTitle}>{n}x</Text>
                  <View style={{ flex: 1 }} />
                  <View style={[m.radioOuter, n === installments && { borderColor: "#000" }]}>
...
const m = StyleSheet.create({
  ...
  h1: { color: "#000", fontSize: 20, fontWeight: "700", letterSpacing: -0.2 },
  backBtn: { minWidth: 64, height: 44, justifyContent: "center" },
  backText: { color: "#000", fontSize: 15, fontWeight: "900", letterSpacing: -0.2 },
  ...
  input: { height: 48, borderWidth: 1, borderColor: "#E1E5F0", borderRadius: 14, paddingHorizontal: 14, color: "#000", backgroundColor: "#fff", fontSize: 15, fontWeight: "500", marginTop: 8 },

  innerDivider: { height: StyleSheet.hairlineWidth, backgroundColor: "rgba(15,23,42,0.08)", marginVertical: 10 },

  option: { borderWidth: 1, borderColor: "rgba(15,23,42,0.15)", borderRadius: 14, paddingHorizontal: 14, paddingVertical: 14, flexDirection: "row", alignItems: "center", backgroundColor: "#fff" },
  ...
  ctaBtn: { height: 54, borderRadius: 14, borderWidth: 1, borderColor: "#000", alignItems: "center", justifyContent: "center", backgroundColor: "#000" },
