...
export function OwnerCartScreen() {
  ...
  const previewQ = useQuery({
    queryKey: ["owner-cart-preview", itemsKey, appliedCoupon],
  ...
  const applyCouponMut = useMutation({
    ...
    onSuccess: (data) => {
      ...
      qc.setQueryData(["owner-cart-preview", itemsKey, normalized], data);
    },
    onError: (e: any) => {
      const fe = friendlyError(e);
      setModal({ title: "Cupom inválido", message: fe.message || "Cupom inválido." });
    ...
    setConfirm({
      ...
      actions: [
        ...
        { text: "Validar", onPress: () => applyCouponMut.mutate(typed) },
        {
          ...
          style: "destructive",
          onPress: () =>
            nav.navigate(OWNER_SCREENS.ShippingMethod, {
              items: itemsPayload,
              couponCode: appliedCoupon || undefined,
            }),
        },
