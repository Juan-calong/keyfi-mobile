// screens/customer/CustomerBoletoPayerFormScreen.tsx
...
export function CustomerBoletoPayerFormScreen({ route, navigation }: any) {
  ...
  if (!url) {
    showBanner("Boleto", "Boleto gerado, mas não recebi o link para abrir.");
    navigation.goBack();
    return;
  }

  navigation.replace(CUSTOMER_SCREENS.BoletoWebView, { url });
},
    onError: (e: any) => {
      const fe: any = friendlyError(e);

      if (fe?.status === 429 && fe?.retryAfterSec) {
        const cooldown = formatCooldown(fe.retryAfterSec);
        setModal({
          title: "Muitas tentativas",
          message: `Tente novamente em ${cooldown}.`,
        });
        return;
      }

      // validação local: preferir mensagem direta do throw (não precisa “Erro desconhecido”)
      const msg = String(fe?.message || "Falha ao gerar boleto.");
      setModal({ title: String(fe?.title || "Erro"), message: msg });
    },
  });
  ...
  const onSubmit = () => {
    ...
    try {
      ...
      // libera no próximo tick (evita double tap no mesmo frame)
      setTimeout(() => {
          ...
          <Container style={{ flex: 1, paddingTop: 6 }}>
            ...
            >
              ...
              <Card title="Endereço">
                ...
                <View style={s.row}>
                  <View style={s.col}>
                    <Input label="Número" value={streetNumber} onChangeText={setStreetNumber} placeholder="Ex.: 123" returnKeyType="next" />
                  </View>

                  <View style={s.gap} />

                  <View style={s.col}>
                    <Input label="Bairro" value={neighborhood} onChangeText={setNeighborhood} placeholder="Ex.: Centro" returnKeyType="next" />
                  </View>
                </View>
...
const s = StyleSheet.create({
  ...
  backBtn: { minWidth: 72, height: 44, paddingHorizontal: 8, borderRadius: 14, flexDirection: "row", alignItems: "center", gap: 4 },
  backText: { color: "#0F172A", fontSize: 15, fontWeight: "800", letterSpacing: -0.2 },
  navTitle: { color: "#0F172A", fontSize: 18, fontWeight: "900", letterSpacing: -0.2 },

  scroll: { paddingTop: 10, paddingHorizontal: HPAD, paddingBottom: 0, gap: 12 },
  ...
  cardTitle: { color: "#0F172A", fontSize: 16, fontWeight: "900", letterSpacing: -0.2 },

  field: { gap: 6, marginTop: 10 },
  label: { fontSize: 12.5, fontWeight: "800", color: "#334155" },
  input: {
    height: 48,
    borderRadius: 14,
  ...
  securityRow: { marginTop: 14, flexDirection: "row", alignItems: "center", gap: 8 },
  securityText: { fontSize: 13.5, color: "#166534", fontWeight: "900" },
  ...
  ctaDivider: { height: StyleSheet.hairlineWidth, backgroundColor: "rgba(15, 23, 42, 0.10)", width: "100%", marginBottom: 10 },
  ctaBtn: {
    height: 54,
    borderRadius: 16,
    alignItems: "center",
